...
class BIRS_Calendar_View extends BIRS_Admin_View
{
    ...
    {
        parent::admin_init();
        add_action('wp_ajax_birs_query_appointments',
                array(
                        &$this,
                        'ajax_query_appointments'
                ));
        add_action('wp_ajax_birs_render_edit_form',
                array(
                        &$this,
                        'ajax_render_edit_form'
                ));
        add_action('wp_ajax_birs_save_appointment',
                array(
                        &$this,
                        'ajax_save_appointment'
                ));
        add_action('wp_ajax_birs_delete_appointment',
                array(
                        &$this,
                        'ajax_delete_appointment'
                ));
        add_filter('birchschedule_general_client_section_admin_edit',
                array(
                        $this,
                        'get_general_client_section_html'
                ), 10, 2);
        add_filter('birchschedule_client_details_admin_edit',
                array(
                        $this,
                        'get_client_details_html'
                ), 10, 3);
        add_filter('birchschedule_appointment_details_admin_edit',
                array(
                        $this,
                        'get_appointment_details_html'
                ), 10, 2);
        add_filter('birchschedule_validate_booking_info_admin',
                array(
                        $this,
                        'validate_data'
                ));
                ...
                array(
                        'labels' => array(
                                ...
                                'not_found_in_trash' => __(
                                        'No Appointments found in trash',
                                        'birchschedule'),
                                'parent' => __('Parent Appointment',
                                        'birchschedule')
                        ),
                        'description' => __(
                                'This is where appointments are stored.',
                                'birchschedule'),
                        'public' => false,
                        'show_ui' => false,
        ...
        if (isset($_REQUEST['page']) &&
                 $_REQUEST['page'] == 'birchschedule_calendar') {
    ...
    function get_admin_scripts ()
    {
        ...
        $delete_button_html = '<a id="delete_appointment" class="submitdelete deletion" href="javascript:void(0)">' .
                 __('Delete', 'birchschedule') . '</a>';
        
        $add_appointment_title = '<div>' . $save_button_html . '<span>' .
                 __('Add Appointment', 'birchschedule') . '</span>' . '</div>';
        $edit_appointment_title = '<div class="submitbox">' . $save_button_html .
                 $delete_button_html . '<span>' .
                 __('Edit Appointment', 'birchschedule') . '</span>' . '</div>';
        $gmt_offset = $this->get_util()->get_gmt_offset();
        $fc_time_format = $this->get_util()->date_time_format_php_to_fullcalendar(
                get_option('time_format', 'g:i a'));
        $params = array(
                'ajax_url' => admin_url('admin-ajax.php'),
                'default_calendar_view' => apply_filters(
                        'birchschedule_default_calendar_view', 'agendaWeek'),
                ...
                'i18n' => array(
                        ...
                        'loading_appointments' => __('Loading appointments...',
                                'birchschedule')
                )
        );
        $scripts[] = array(
                'birs_admin_calendar',
                'birs_calendar_params',
                $params
        );
        $jquery_date_format = $this->get_util()->date_time_format_php_to_jquery(
        ...
        $scripts[] = array(
                'birs_admin_appointment_edit',
                'birs_appointment_edit_params',
                $appointment_edit_params
        );
        return $scripts;
    }
    ...
    function get_admin_styles ()
    {
        return array(
                'jquery-ui-bootstrap',
                'birs_lib_fullcalendar',
                'birchschedule_admin_styles',
                'select2',
                'jgrowl'
        );
    }
    ...
    function ajax_query_appointments ()
    {
        ...
        foreach ($appointments as $appointment) {
            ...
            $duration = get_post_meta($post_id, '_birs_appointment_duration',
                    true);
            $duration = intval($duration);
            $price = get_post_meta($post_id, '_birs_appointment_price', true);
            $time_start = get_post_meta($post_id, '_birs_appointment_timestamp',
                    true);
            $time_end = $time_start + $duration * 60;
            $time_start = $this->get_util()
                ->get_wp_datetime($time_start)
                ->format('c');
            $time_end = $this->get_util()
                ->get_wp_datetime($time_end)
                ->format('c');
            $apmt = array(
                    'id' => $appointment->ID,
...
<div class="birchschedule wrap">
	<div id="birs_calendar_toolbar">
		<h2 id="birs_calendar_title">
				...
				id="birs_add_appointment" class="add-new-h2">
                           <?php _e('Add Appointment', 'birchschedule'); ?>
                    </a> <a href="javascript:void(0)"
				id="birs_calendar_refresh" class="add-new-h2">
                           <?php _e('Refresh', 'birchschedule'); ?>
                    </a>
		</h2>
		<div id="birs_calendar_filter">
			<label><?php _e('Location', 'birchschedule'); ?></label> <select
				id="birs_calendar_location">
                        <?php $this->render_location_options(); ?>
                    </select> <label><?php _e('Staff', 'birchschedule'); ?></label>
	...
	<div id="birs_add_new_dialog"></div>
</div>
<?php
    }
    ...
    function get_client_details_html ($html, $client_id)
    {
        ...
        if ($client_id) {
            $client_title = get_post_meta($client_id, '_birs_client_title',
                    true);
...
<table>
	<tr>
		...
		<td><select name="birs_client_title" id="birs_client_title">
                        <?php $this->get_util()->render_html_options($client_titles, $client_title); ?>
                    </select> <input type="hidden"
			name="birs_client_fields[]" value="_birs_client_title" /></td>
	</tr>
	<tr>
		<th><label><?php _e('Address', 'birchschedule'); ?></label></th>
		<td><input type="text" name="birs_client_address1"
			id="birs_client_address1" value="<?php echo $address1; ?>"> <input
			type="hidden" name="birs_client_fields[]"
			value="_birs_client_address1" /></td>
	</tr>
	<tr>
		<th><label></label></th>
		<td><input type="text" name="birs_client_address2"
			id="birs_client_address2" value="<?php echo $address2; ?>"> <input
			type="hidden" name="birs_client_fields[]"
			value="_birs_client_address2" /></td>
	</tr>
	<tr>
		<th><label><?php _e('City', 'birchschedule') ?></label></th>
		<td><input type="text" name="birs_client_city" id="birs_client_city"
			value="<?php echo $city; ?>"> <input type="hidden"
			name="birs_client_fields[]" value="_birs_client_city" /></td>
	</tr>
	<tr>
		<th><label><?php _e('State/Province', 'birchschedule') ?></label></th>
		<td><select name="birs_client_state" id="birs_client_state">
                        <?php $this->get_util()->render_html_options($states, $state); ?>
                    </select> <input type="hidden"
			name="birs_client_fields[]" value="_birs_client_state" /> <input
			type="text" name="birs_client_province" id="birs_client_province"
			value="<?php echo esc_attr($province); ?>" style="display: none;"> <input
			type="hidden" name="birs_client_fields[]"
			value="_birs_client_province" /></td>
	</tr>
	<tr>
		<th><label><?php _e('Country', 'birchschedule'); ?></label></th>
		<td><select name="birs_client_country" id="birs_client_country">
                        <?php
        $this->get_util()->render_html_options($countries, $country,
                $default_country);
        ?>
                    </select> <input type="hidden"
			name="birs_client_fields[]" value="_birs_client_country" /></td>

	</tr>
	<tr>
		<th><label><?php _e('Zip Code', 'birchschedule'); ?></label></th>
		<td><input type="text" name="birs_client_zip" id="birs_client_zip"
			value="<?php echo $zip; ?>"> <input type="hidden"
			name="birs_client_fields[]" value="_birs_client_zip" /></td>
	</tr>
</table>
<?php
    ...
    function get_appointment_details_html ($html, $appointment_id)
    {
        $notes = '';
        if ($appointment_id) {
            $notes = get_post_meta($appointment_id, '_birs_appointment_notes',
                    true);
        }
        ob_start();
        ?>
<table>
	<tr>
        ...
        ?></label></th>
		<td><textarea id="birs_appointment_notes"
				name="birs_appointment_notes"><?php echo $notes; ?></textarea> <input
			type="hidden" name="birs_appointment_fields[]"
			value="_birs_appointment_notes" /></td>
	</tr>
</table>
<?php
        $content = ob_get_clean();
    ...
    function ajax_render_edit_form ()
    {
        ...
        if (isset($_GET['birs_appointment_id'])) {
            ...
            $timestamp = get_post_meta($appointment_id,
                    '_birs_appointment_timestamp', true);
            ...
            $price = get_post_meta($appointment_id, '_birs_appointment_price',
                    true);
            $service_id = get_post_meta($appointment_id,
                    '_birs_appointment_service', true);
            $staff_id = get_post_meta($appointment_id,
                    '_birs_appointment_staff', true);
            $client_id = get_post_meta($appointment_id,
                    '_birs_appointment_client', true);
        }
        ?>
<div id="birs_appointment_edit">
	...
	<form id="birs_appointment_form">
		<div class="wrap" id="birs_appointment_info">
                    ...
                    <input type="hidden" name="birs_appointment_id"
				id="birs_appointment_id" value="<?php echo $appointment_id; ?>"> <input
				type="hidden" name="birs_appointment_location"
				id="birs_appointment_location" value="<?php echo $location_id; ?>">
			<div id="birs_general_section_appointment">
				<table>
					<tr>
						<th style="width: 16%;"><label><?php _e('Service', 'birchschedule'); ?></label>
						</th>
						<td><select id="birs_appointment_service"
							name="birs_appointment_service"><?php $service_id = $this->render_service_options($service_id); ?></select>
						</td>
					</tr>
					<tr>
						<th><label><?php _e('Staff', 'birchschedule'); ?></label></th>
						<td><select id="birs_appointment_staff"
							name="birs_appointment_staff"><?php $this->render_staff_options($location_id, $service_id, $staff_id); ?></select>
						</td>
					</tr>
					<tr class="error">
						<th></th>
						<td><label id="birs_appointment_service_error" class="error"></label>
						</td>
					</tr>
					<tr>
						<th><label><?php _e('Date & Time', 'birchschedule'); ?></label></th>
						<td><input id="birs_appointment_datepicker"
							name="birs_appointment_datepicker" type="text"
							value="<?php echo $date4picker ?>"> <input
							id="birs_appointment_date" name="birs_appointment_date"
							type="hidden" value="<?php echo $date ?>"> <select
							id="birs_appointment_time" name="birs_appointment_time">
                                        <?php $this->render_time_options($time); ?>
                                    </select></td>
					</tr>
					<tr class="error">
						<th></th>
						<td><label id="birs_appointment_datetime_error" class="error"></label>
						</td>
					</tr>
					<tr>
						<th><label><?php echo apply_filters('birchschedule_price_label', __('Price', 'birchschedule')); ?></label>
						</th>
						<td><input type="text" id="birs_appointment_price"
							name="birs_appointment_price" value="<?php echo $price; ?>"></td>
					</tr>
				</table>
			</div>
        ...
        echo apply_filters('birchschedule_general_client_section_admin_edit',
                '', $client_id);
        ?>
                    </div>
		</div>
		<div id="birs_appointment_details">
                    <?php
        echo apply_filters('birchschedule_appointment_details_admin_edit', '',
                $appointment_id);
        ?>
                </div>
		<div id="birs_client_details">
                    <?php
        echo apply_filters('birchschedule_client_details_admin_edit', '',
                $client_id,
                array(
                        'client_name_first',
                        'client_name_last',
...
<?php
    ...
    function get_general_client_section_html ($html, $client_id)
    {
        ...
        $phone = '';
        if ($client_id) {
            $first_name = get_post_meta($client_id, '_birs_client_name_first',
                    true);
            $last_name = get_post_meta($client_id, '_birs_client_name_last',
                    true);
...
<table style="margin-left: 9px;">
	<tr>
		<th><label><?php _e('First Name', 'birchschedule'); ?></label></th>
		<td><input type="text" id="birs_client_name_first"
			name="birs_client_name_first" value="<?php echo $first_name; ?>"> <input
			type="hidden" name="birs_client_fields[]"
			value="_birs_client_name_first" /></td>
	</tr>
	<tr class="error">
		<th></th>
		<td><label id="birs_client_name_first_error" class="error"></label></td>
	</tr>
	<tr>
		<th><label><?php _e('Last Name', 'birchschedule'); ?></label></th>
		<td><input type="text" id="birs_client_name_last"
			name="birs_client_name_last" value="<?php echo $last_name; ?>"> <input
			type="hidden" name="birs_client_fields[]"
			value="_birs_client_name_last" /></td>
	</tr>
	<tr class="error">
		<th></th>
		<td><label id="birs_client_name_last_error" class="error"></label></td>
	</tr>
	<tr>
		<th><label><?php _e('Email', 'birchschedule'); ?></label></th>
		<td><input type="text" id="birs_client_email" name="birs_client_email"
			value="<?php echo $email; ?>"> <input type="hidden"
			name="birs_client_fields[]" value="_birs_client_email" /></td>
	</tr>
	<tr class="error">
		<th></th>
		<td><label id="birs_client_email_error" class="error"></label></td>
	</tr>
	<tr>
		<th><label><?php _e('Phone', 'birchschedule'); ?></label></th>
		<td><input type="text" id="birs_client_phone" name="birs_client_phone"
			value="<?php echo $phone; ?>"> <input type="hidden"
			name="birs_client_fields[]" value="_birs_client_phone" /></td>
	</tr>
	<tr class="error">
		<th></th>
		<td><label id="birs_client_phone_error" class="error"></label></td>
	</tr>
</table>
<?php
        $html = ob_get_clean();
    ...
    function ajax_save_appointment ()
    {
        ...
        $permitted = check_ajax_referer("birs_save_appointment-$appointment_id",
                '_wpnonce', false);
        if ($permitted) {
            $errors = apply_filters('birchschedule_validate_booking_info_admin',
                    array());
            if (! $errors) {
...
<?php
    ...
    function validate_appointment_info ()
    {
        ...
        if (! isset($_POST['birs_appointment_staff']) ||
                 ! isset($_POST['birs_appointment_service'])) {
            $errors['birs_appointment_service'] = __(
                    'Please select a service and a ' . $staff_text,
                    'birchschedule');
        }
        if (! isset($_POST['birs_appointment_date']) ||
                 ! $_POST['birs_appointment_date'] ||
                 ! isset($_POST['birs_appointment_time']) ||
                 ! $_POST['birs_appointment_time']) {
            $errors['birs_appointment_datetime'] = __('Date & time is required',
                    'birchschedule');
        } else {
            $datetime = array(
            ...
            if (! $datetime) {
                $errors['birs_appointment_datetime'] = __(
                        'Date & time is incorrect', 'birchschedule');
            } else {
    ...
    function validate_data ()
    {
        ...
        if (! $_POST['birs_client_name_first']) {
            $errors['birs_client_name_first'] = __('First name is required',
                    'birchschedule');
        }
        if (! $_POST['birs_client_name_last']) {
            $errors['birs_client_name_last'] = __('Last name is required',
                    'birchschedule');
        }
        if (! $_POST['birs_client_email']) {
            $errors['birs_client_email'] = __('Email is required',
                    'birchschedule');
        } else
            if (! is_email($_POST['birs_client_email'])) {
                $errors['birs_client_email'] = __('Email is incorrect',
                        'birchschedule');
            }
        if (! $_POST['birs_client_phone']) {
            $errors['birs_client_phone'] = __('Phone is required',
                    'birchschedule');
        }
    ...
    function get_service_length ($service_id, $client_id = 0)
    {
        ...
        $padding_type = get_post_meta($service_id, '_birs_service_padding_type',
                true);
